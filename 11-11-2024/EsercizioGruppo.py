# Creazione del file di testo con contenuto di esempio Lorem Ipsum
lorem_text = """
...
# Salviamo il testo in un file txt
file_path = 'lorem_ipsum.txt'
with open(file_path, 'w') as file:
    file.write(lorem_text)

# Funzione per leggere il file e contare parole, righe e caratteri
def count_file_contents(file_path):
    ...
    num_chars = sum(len(line.replace("/n","")) for line in lines)
    
    return num_lines, num_words, num_chars

# Eseguiamo la funzione per ottenere le informazioni richieste
num_lines, num_words, num_chars = count_file_contents(file_path)


print(f"Il file contiene {num_lines} righe, {num_words} parole e {num_chars} caratteri.")
